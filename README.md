...
![Levolor Motorized Blind Smart Home Controller Hero Shot](assets/pcb_schematic_horizontal.jpg)
...
To use this bridge, your Arduino "Thing" must have the following properties configured:
- `up` (Boolean, Switch/Push): Triggers the blinds to open.
- `down` (Boolean, Switch/Push): Triggers the blinds to close.
- `debug_log` (String, Read-Only): Used by the ESP32 to report battery health and diagnostic logs.

### 2. Bridge Installation
1. **Clone the repo**
    ```bash
    git clone https://github.com/yourusername/Levolor Motorized Blind Smart Home Controller.git
    cd Levolor Motorized Blind Smart Home Controller
    npm install
    ```
...
2.  **Configure Credentials**
    ...
    # Secure your bridge (optional but recommended)
    AUTH_TOKEN=your-secret-local-token
    
    # Arduino IoT Cloud Credentials (get these from Arduino IoT Cloud integrations)
...
3.  **Map Your Devices**
    Edit `devices.config.json` to map logical names to Arduino Thing/Property IDs.
    ```json
    {
      "devices": {
        "office": {
...
Custom PCB design using Seeed XIAO ESP32-C3 with battery charging (BQ25886) and power management (TPS62152).

See [`hardware/README.md`](hardware/README.md) for:
- PCB Gerber files (ready for manufacturing)
